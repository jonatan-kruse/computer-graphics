...
void FPSCamera<T, P>::Update(std::chrono::microseconds deltaTime, InputHandler &ih, bool ignoreKeyEvents, bool ignoreMouseEvents) {
    ...
    if (!ih.IsKeyboardCapturedByUI() && !ignoreKeyEvents) {
        ...
        if ((ih.GetKeycodeState(GLFW_KEY_D) & PRESSED))
            strafe += 1.0f;
        if ((ih.GetKeycodeState(GLFW_KEY_Q) & PRESSED))
            levitate -= 1.0f;
        if ((ih.GetKeycodeState(GLFW_KEY_E) & PRESSED))
            levitate += 1.0f;

        T const movementModifier = ((ih.GetKeycodeState(GLFW_KEY_LEFT_CONTROL) & PRESSED)) ? 0.25f : ((ih.GetKeycodeState(GLFW_KEY_LEFT_SHIFT) & PRESSED)) ? 4.0f
                                                                                                                                                           : 1.0f;
