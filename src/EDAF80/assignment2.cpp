...
bool show_interpolation = !show_sphere;
...
void edaf80::Assignment2::run() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (interpolate) {
            ...
            float x = elapsed_time_s - floor(elapsed_time_s);                                 // Fractional part of time
        ...
        if (opened) {
            ...
            if (selection_result.was_selection_changed) {
                if (show_interpolation) {
                    ...
                    tessellated_sphere.set_program(selection_result.program, set_uniforms);
                }
            }
            ImGui::Separator();
